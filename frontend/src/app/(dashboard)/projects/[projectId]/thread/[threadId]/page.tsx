...
export default function ThreadPage({
  ...
  useEffect(() => {
    if (projectName) {
      document.title = `${projectName} | Quriosity Q`;
      ...
      if (metaDescription) {
        metaDescription.setAttribute(
          'content',
          `${projectName} - Interactive agent conversation powered by Quriosity Q`,
        );
      }

      const ogTitle = document.querySelector('meta[property="og:title"]');
      if (ogTitle) {
        ogTitle.setAttribute('content', `${projectName} | Quriosity Q`);
      }
  ...
  useEffect(() => {
    if (initialLoadCompleted && subscriptionData) {
      const hasSeenUpgradeDialog = localStorage.getItem('q_upgrade_dialog_displayed');
  ...
  const handleDismissUpgradeDialog = () => {
    ...
    localStorage.setItem('q_upgrade_dialog_displayed', 'true');
  };
